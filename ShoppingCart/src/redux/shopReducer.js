    ...
    switch (action.type) {
        case actionTypes.ADD_TO_CART:
            ...
            return{
                ...state,
                cart: inCart?
                        state.cart.map((product)=> product.id === action.payload.id? {...product,qty:Number.parseInt(product.qty)+1}:product):
        ...
        case actionTypes.UPDATE_QTY:
            return{
                ...state,
                cart: state.cart.map((product)=> product.id === action.payload.id? {...product,qty:Number.parseInt(action.payload.qty)}:product)   
            }
